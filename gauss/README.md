...
Gauss คือเว็บแอปสำหรับจัดการไฟล์แบบรวมทุกอย่างในหน้าเดียว โดยเน้นความเป็นส่วนตัวและการประมวลผลฝั่งเบราว์เซอร์ ผู้ใช้สามารถอัปโหลดไฟล์ เลือกเครื่องมือ ปรับตัวเลือก แล้วดาวน์โหลดผลลัพธ์จริงได้ทันที

เครื่องมือหลัก:

- รูปภาพ: แปลงไฟล์ ปรับขนาด หมุน พลิก และใส่เอฟเฟกต์
- PDF: ตรวจสอบ รวม แยก บีบอัด/บันทึกใหม่ และดึงหน้า
- แปลงไฟล์: ส่งออกเป็น PDF รูปภาพ TXT JSON CSV Data URL ZIP หรือไฟล์ต้นฉบับที่เปลี่ยนชื่อ
- บีบอัด: สร้าง ZIP พร้อมจัดโฟลเดอร์และ manifest
- งานชุด: เปลี่ยนชื่อ จัดเรียง ติดแท็ก และเปลี่ยนตัวพิมพ์
- OCR: อ่านข้อความจากรูปภาพด้วย Tesseract.js
- เมตาดาตา: สร้างรายงานข้อมูลไฟล์
- แฮช: สร้าง checksum สำหรับตรวจสอบไฟล์
- ข้อความ: ล้าง แปลง นับ และส่งออกข้อความ
...
The app can be deployed to platforms that support Next.js, such as Vercel. Build with:

```bash
npm run build
```

Then deploy the `gauss` project according to your hosting provider's Next.js instructions.
